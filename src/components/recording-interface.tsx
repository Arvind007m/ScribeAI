...
export function RecordingInterface() {
      ...
      if (audioSource === 'tab-audio') {
        ...
        try {
          ...
          toast({
            ...
            description: "Note: Real-time transcription only works with microphone. Tab audio is being recorded.",
            variant: "default",
          });
  ...
  const renderControls = () => {
    switch (sessionState) {
      case 'idle':
        return <Button onClick={handleStart} size="lg"><Play className="mr-2 h-4 w-4" /> Start Session</Button>;
      case 'recording':
        return (
          <div className="flex gap-4">
            <Button onClick={handlePause} variant="secondary" size="lg"><Pause className="mr-2 h-4 w-4" /> Pause</Button>
            <Button onClick={handleStop} variant="destructive" size="lg"><StopCircle className="mr-2 h-4 w-4" /> Stop</Button>
          </div>
        );
      case 'paused':
        return (
          <div className="flex gap-4">
            <Button onClick={handleResume} size="lg"><Play className="mr-2 h-4 w-4" /> Resume</Button>
            <Button onClick={handleStop} variant="destructive" size="lg"><StopCircle className="mr-2 h-4 w-4" /> Stop</Button>
          </div>
        );
      case 'processing':
        return <Button disabled size="lg"><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Processing...</Button>;
      case 'completed':
        return (
          <div className="flex gap-4">
            <Button onClick={exportTranscript} variant="outline" size="lg"><Download className="mr-2 h-4 w-4" /> Export</Button>
            <Button onClick={handleReset} size="lg"><Play className="mr-2 h-4 w-4" /> Start New Session</Button>
          </div>
        );
    }
  };
    ...
    <div className="flex flex-col items-center justify-center p-6 w-full max-w-4xl mx-auto">
        ...
        {sessionState === 'idle' && (
            <div className="flex flex-col items-center gap-6">
                ...
                <p className="text-muted-foreground text-center">Select your audio source and start a new session.</p>
                ...
                >
                    <div className="flex items-center space-x-2">
                        ...
                        <Label htmlFor="microphone" className="flex items-center gap-2 text-lg cursor-pointer">
        ...
        {(isRecordingActive || sessionState === 'processing' || sessionState === 'completed') && (
            <Card className="w-full">
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <Badge variant={sessionState === 'recording' ? "destructive" : "secondary"} className="capitalize">
                        {sessionState === 'recording' && <div className="w-2 h-2 rounded-full bg-red-500 mr-2 animate-pulse"></div>}
                        {sessionState}
                    </Badge>
                </CardHeader>
                <CardContent>
